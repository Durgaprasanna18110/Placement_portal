    ...
    useEffect(() => {
        const fetchSingleJob = async () => {
            try {
                ...
                if (res.data.success) {
                    console.log("Fetched Job Data:", res.data.job); // Debugging
    ...
    return (
        <div className='max-w-7xl mx-auto my-10'>
            <div className='flex items-center justify-between'>
                <div>
                    <h1 className='font-bold text-xl'>{singleJob?.title || "Job Title Not Available"}</h1>
                    <div className='flex items-center gap-2 mt-4'>
                        <Badge className={'text-blue-700 font-bold'} variant="ghost">
                            {singleJob?.postion ? `${singleJob.postion} Positions` : "N/A"}
                        </Badge>
            ...
            <div className='my-4'>
                ...
                <h1 className='font-bold my-1'>Description: 
                    <span className='pl-4 font-normal text-gray-800'>{singleJob?.description || "N/A"}</span>
                </h1>
                <h1 className='font-bold my-1'>Experience: 
                    <span className='pl-4 font-normal text-gray-800'>{singleJob?.experience ? `${singleJob.experience} yrs` : "N/A"}</span>
                </h1>
                {singleJob?.minCGPA && (
                    <h1 className='font-bold my-1'>Minimum CGPA: 
                        <span className='pl-4 font-normal text-gray-800'>{singleJob.minCGPA.toFixed(2)}</span>
                    </h1>
                )}
                <h1 className='font-bold my-1'>Salary: 
                    <span className='pl-4 font-normal text-gray-800'>{singleJob?.salary ? `${singleJob.salary} LPA` : "N/A"}</span>
                </h1>
                <h1 className='font-bold my-1'>Total Applicants: 
                    <span className='pl-4 font-normal text-gray-800'>{singleJob?.applications?.length ?? "N/A"}</span>
                </h1>
